...
//		options.addOption( "s", "service", true, "service to run" );
		...
		if (line.hasOption("usage")) {
			...
			formatter.printHelp( "Bisdw", options );
			System.exit(0);
		}
	...
	private int daemon() {
		...
		for (ETLJobConfig jobentry: schedulejobs) {
			...
			for (Trigger trigger: jobentry.getSchedules()) {
				try {
					JobDetail job = newJob(ETLJobExecute.class)
				      .withIdentity(jobentry.getETLJob().getName()+(jobid++), jobentry.getETLJob().getName())
				      .withDescription(jobentry.getETLJob().getName())
				      .usingJobData(jobmap)
				      .build();
